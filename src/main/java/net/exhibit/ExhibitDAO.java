...
con = dbopen.getConnection();
			...
			sql.append(" SELECT excode, bcode, explace, exname, contents, author, exstart, exend, excnt, price, tel ");
			sql.append(" FROM exh_info ");
	...
	public int update(ExhibitDTO dto) {
		...
		try {
			...
			sql.append(" SET excode=?, bcode = ?, explace=?, exname=?, contents=?, author=?, exstart=?, exend=?, excnt=?, price=?, tel=? , filename=? ");
			sql.append(" WHERE excode = ? ");
			...
			pstmt.setString(7,dto.getExstart());
			pstmt.setString(8,dto.getExend());
			pstmt.setInt(9,dto.getExcnt());
